...
def state_vp(df):
    ...
    for state in STATES:
        ...
        sl_vp = sl_df.shape[0]
        b_df = vpdf[vpdf["race_type"] == "Ballot Initiative"]
        b_vp = b_df.shape[0]
...
def election_csv_cleaner(location, destination, csv_name):
    ...
    df.to_csv(destination + csv_name,  index = False)

def candidate_csv_cleaner(location, destination, csv_name):
...
def csv_reader(filename):
    ...
    df = pd.read_csv(destination + p_file)
    return df
...
for df_file in file_list:
    election_csv_cleaner(location, destination, df_file)

candidate_csv_cleaner(location, destination, c_file)

#Read in the cleaned CSVs
p = pd.read_csv(destination + p_file, dtype=str)
s = pd.read_csv(destination + s_file, dtype=str)
h = pd.read_csv(destination + h_file, dtype=str)
g = pd.read_csv(destination + g_file, dtype=str)
su = pd.read_csv(destination + su_file, dtype=str)
sl = pd.read_csv(destination + sl_file, dtype=str)
b = pd.read_csv(destination + b_file, dtype=str)
...
svp_df.to_csv("static/data/calculated_files/aggregate.csv",  index = False)
merge_df.to_csv("static/data/calculated_files/elections.csv",  index = False)
